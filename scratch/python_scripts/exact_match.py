...
# Replace the wrapped HIW with exactly what the client provided
old_hiw_pattern = re.compile(r'<section class="hiw">.*?</section>', re.DOTALL)
new_hiw = """<section class="hiw">
...
content = re.sub(old_hiw_pattern, new_hiw, content)

# Replace the wrapped CATS with exactly what the client provided
old_cats_pattern = re.compile(r'<section class="cats center">.*?</section>', re.DOTALL)
new_cats = """<section class="cats center">
...
content = re.sub(old_cats_pattern, new_cats, content)

# Replace the custom CSS grid max-width/padding to exactly what the client had (with Webflow container styling removed from my injection)
# Wait, I'll just find and replace the CSS block again using regex and put the exact CSS from the file
old_css_pattern = re.compile(r'/\* ============ 2\. HOW IT WORKS.*?\.center \{ text-align: center; \}\n*', re.DOTALL)
...
exact_css = """
  ...
  .hiw-grid { display: grid; grid-template-columns: repeat(5, 1fr); gap: 16px; padding: 0 40px; max-width: 1600px; margin: 0 auto; }
  @media (max-width: 1000px) { .hiw-grid { grid-template-columns: repeat(2, 1fr); padding: 0 20px; } .hiw-grid .flip:last-child { grid-column: 1 / -1; } }
  @media (max-width: 560px)  { .hiw-grid { grid-template-columns: 1fr; padding: 0 16px;} .flip, .flip-inner { min-height: 240px; } }
...
"""
content = content.replace('</style>', exact_css + '\n</style>', 1)

with open('public/main.html', 'w', encoding='utf-8') as f:
    f.write(content)

print("Restored exact HTML and CSS from client")
