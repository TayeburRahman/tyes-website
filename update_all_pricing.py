...
for filepath in files_to_update:
    ...
    if "4. PRICING — grouped" not in content:
        content = content.replace('</head>', exact_css + '\n</head>', 1)
        print(f"Added CSS to {filepath}")
    
    # Replace the <main> block
    main_pattern = re.compile(r'<main class="main-wrapper">.*?</main>', re.DOTALL)
    new_main_content = f'<main class="main-wrapper">\n{pricing_html}\n</main>'
    content = re.sub(main_pattern, new_main_content, content)
    
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(content)
    print(f"Replaced pricing content in {filepath}")
