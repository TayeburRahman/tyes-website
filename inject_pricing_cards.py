...
new_cards_html = """
  ...
  <div class="custom">
    <div>
      ...
      <h3 style="font-size:26px; margin: 0; padding: 0;">Custom. <span class="accent">Not just visuals.</span></h3>
      <p class="desc" style="margin: 0; padding: 0; margin-top: 8px;">Custom number of images · Custom revisions · Dedicated manager · + FREE Brand Strategy Snapshot · + 30-min strategy call</p>
    </div>
...
for filepath in files_to_update:
    ...
    # Find where <div class="price-grid-main"> starts
    start_str = '<!-- ======= 5 MAIN TIERS ======= -->\n                <div class="price-grid-main">'
    start_idx = content.find(start_str)
    
    if start_idx != -1:
        # Find where the deep dive section starts to know where to end the replacement
        end_str = '<!-- ======= DEEP DIVE SECTION ======= -->'
        end_idx = content.find(end_str, start_idx)
        
        if end_idx != -1:
            # We want to replace everything from start_idx up to end_idx with new_cards_html
            old_block = content[start_idx:end_idx]
            content = content.replace(old_block, start_str.split('\n')[0] + '\n' + new_cards_html + '\n\n                ')
            print(f"Successfully replaced pricing cards in {filepath}")
        else:
            print(f"Could not find end of price-grid-main in {filepath}")
    else:
        print(f"Could not find start of price-grid-main in {filepath}")
